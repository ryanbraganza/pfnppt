...
router.post('/action/handleMessage', cors(), async (req, res, next) => {
  ...
  try {
    ...
    logger.info(`${loggerInfoName} echo'd the message with response ${util.format(response)}`);
    res.sendStatus(204);
  } catch (err) {
    res.sendStatus(500);
...
router.post('/inlineMessageAction', async (req, res, next) => {
  ...
  try {
    ...
    let messageWithInlineActions = helpers.actions.messageWithInlineActions();
    ...
    stride.api.messages
      ...
      .then(messageResponse => {
        res.sendStatus(204);
        logger.info(`${loggerInfoName} outgoing successful ${util.format(messageResponse)}`);
      })
      .catch(err => {
        logger.error(`${loggerInfoName} sending the message found error: ${err}`);
      });
...
router.post('/action/handleInlineMessageAction', cors(), async (req, res, next) => {
  ...
  try {
    ...
    let replacementMessage = helpers.actions.inlineActionResponseMessage(userId, reason);
    ...
    stride.api.messages
      ...
      .then(messageResponse => {
        res.sendStatus(204);
        logger.info(`${loggerInfoName} sent message successfully ${util.format(messageResponse)}`);
      })
      .catch(err => {
        logger.error(`${loggerInfoName} sending the message found error: ${err}`);
        res.sendStatus(500);
...
router.post('/action/handleInlineMessageSelect', cors(), async (req, res, next) => {
  ...
  try {
    ...
    if (parameters) {
      if (parameters.selectedValue) {
        ...
        if (parameters.selectedValue === 'openRoom') {
          ...
          let createConversation = stride.api.conversations
            .conversation_create(cloudId, opts)
            .then(createRoomResponse => {
              logger.info(`${loggerInfoName} created a room ${util.format(createRoomResponse)}`);
    ...
    let responseMessage = helpers.actions.inlineSelectResponseMessage();
