...
router.post('/addUser', async (req, res) => {
  ...
  try {
    const { id } = await stride.api.conversations.conversation_create(cloudId, roomData);
    const { values } = await stride.api.conversations.conversation_roster(cloudId, conversationId);

    await stride.api.conversations.add_users(cloudId, id, { body: { add: values } });
    res.sendStatus(200);
  } catch (e) {
